...
cv2.imshow('Original Image', image_BGR) # Pay attention! 'cv2.imshow' takes images in BGR format
cv2.waitKey(0) # Waiting for any key being pressed
cv2.destroyWindow('Original Image') # Destroying opened window with name 'Original Image'
...
""" cv2.dnn.blobFromImage() lo que hace es poner la imagen en el estado ideal para que la red pueda 
realizar predicciones sobre ella. De forma más concreta, a esta imagen (o grupo de imágenes 
procesadas), las denominamos blob. """
...
cv2.waitKey(0) # Waiting for any key being pressed
cv2.destroyWindow('Blob Image') # Destroying opened window with name 'Blob Image'
...
start = time.time()
output_from_network = network.forward(layers_names_output) # Get output results only for specified layers 
end = time.time()
...
results = cv2.dnn.NMSBoxes(bounding_boxes, confidences,
                           probability_minimum, threshold) # results = final bounding boxes
